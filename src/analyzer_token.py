...
def calculate_clickbait_score(title):
  ...
  # 3. PROFILOWANE SŁOWA KLUCZOWE (Max 10 pkt)
    ...
    tokens= re.findall(r'\w+',title_upper)
    matches=sum(
        1 for w in sensacja_yt if w in title_upper
    ) 
    score+=min(matches,3) # Wyższa waga za typowo plotkarski/dramowy clickbait
    if any(w in tokens for w in publicystyka):
        score += 2  # Średnia waga za pompę polityczną/newsową
    if any(w in tokens for w in agresja_memy):
...
def calculate_engagement_rate(views, likes):
    try:
        views = int(views)
        likes = int(likes)
...
def calculate_title_features(title):
    letters=sum(l.isalpha() for l in title)
    upper_letters=sum(l.isupper() for l in title)
    ...
    return{
        ...
        "caps_ratio": round(upper_letters/letters,3) if letters else 0
    }
    ...
    for file_path in csv_files:
        ...
        with open(file_path, "r", encoding="utf-8-sig") as f:
            ...
            for row in reader:
                ...
                score, level = calculate_clickbait_score(title)
                eng_rate= calculate_engagement_rate(views,likes)
                features=calculate_title_features(title)
                row.update(features)
